...
-- Table `devshop`.`users`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `devshop`.`users` ;
...
CREATE TABLE IF NOT EXISTS `devshop`.`users` (
  ...
  `roles` VARCHAR(245) NULL COMMENT 'admin, cliente, financeiro',
  PRIMARY KEY (`id`))
ENGINE = InnoDB;
...
-- -----------------------------------------------------
DROP TABLE IF EXISTS `devshop`.`email_tokens` ;
...
CREATE TABLE IF NOT EXISTS `devshop`.`email_tokens` (
  ...
  `used` TINYINT(1) NULL,
  PRIMARY KEY (`id`),
  INDEX `fk_email_tokens_users1_idx` (`users_id` ASC),
